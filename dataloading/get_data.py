...
for conversation in corpus.iter_conversations():
    if its < 10000:
        ...
        upvotes = first_utterance.meta.get(
            "score"
        )  # Adjust this if upvotes are stored differently
        if (lower_bound <= len(first_utterance.text.split()) <= upper_bound) and (
            upvotes >= min_upvotes
        ):
            if detect(first_utterance.text) == "da":
                posts_with_100_tokens_and_upvotes.append(
                    (
                        conversation.meta.get("title", "No Title"),
